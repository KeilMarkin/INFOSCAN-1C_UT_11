	...
	Попытка	
		Для Каждого стзМетодыХранения Из Настройки.МетодыХранения Цикл
			...
			ИначеЕсли стзМетодыХранения.МетодХраненияВГХ = Перечисления.isc_МетодХраненияВГХ.ДопСведенияХарактеристики Тогда
				...
				РС.Значение = сткВходныеПараметры[стзМетодыХранения.ИмяСвойства];	
			КонецЕсли;
			Если СоздаватьНовыеШК И стч.Status = Перечисления.isc_СтатусыРучнаяОбработка.ДанныеЗаписаныВБД Тогда 
...
Процедура СохранитьВБазу(РезультатВопроса, ПараметрыЗаписи) Экспорт	 
	...
	Если РезультатВопроса = КодВозвратаДиалога.Да Тогда
		Если Объект.HistoryOfScaning.НайтиСтроки(Новый Структура("Status",ПредопределенноеЗначение("Перечисление.isc_СтатусыРучнаяОбработка.ШКУстановленВручную"))).Количество() > 0 Тогда
			ПоказатьВопрос(Новый ОписаниеОповещения("СохранитьВБазуНовыеШтрихкоды", ЭтотОбъект), "Обнаружены строки с ручной установкой штрихкода. Создавать новые штрихкоды в базе?", РежимДиалогаВопрос.ДаНет,,, "Предупреждение");
		Иначе	

...
Функция CheckBarcode(Barcode, Status, Comment)
	...
	Если Настройки.Логирование Тогда
		РегистрыСведений.isc_ЛогСобытий.ЗаписатьЛог(Barcode,,,,, "Проверка штрихкода. " +  Status + "." + Comment);
	КонецЕсли;	
	Возврат "";
...
Функция SaveParametrs(Barcode, Status, Comment, Length, Height, Width, Weight, ItemType) 
	...
	Если Результат.Следующий() Тогда
		...
		Для Каждого стзМетодыХранения Из Настройки.МетодыХранения Цикл
			Если стзМетодыХранения.МетодХраненияВГХ = Перечисления.isc_МетодХраненияВГХ.РеквизитыУпаковки Тогда
				...
				Упаковка = ПолучитьУпаковкуИзСоответствияУпаковкам(Результат.Номенклатура, Настройки.СоответствиеУпаковок.Найти(Число(ItemType),"ИдУпаковки"));
				Попытка
				...
				Исключение
					РегистрыСведений.isc_ЛогСобытий.ЗаписатьЛог(Barcode, Length , Height , Width , Weight , "Запись ВГХ. Ошибка получения упаковки." );
				КонецПопытки;
	...
	Если ЗначениеЗаполнено(Настройки) И Настройки.Логирование Тогда
		РегистрыСведений.isc_ЛогСобытий.ЗаписатьЛог(Barcode, Length , Height , Width , Weight , "Запись ВГХ. " +  Status + ". -" + Comment);
	КонецЕсли;
	Возврат "";  
